...
def main():
    model_path = r"D:\.VSCode\Python\AI\model\cnn_model_16t.pth"  # 模型读取路径
    model_spath =r"D:\.VSCode\Python\AI\model\cnn_model_16t.pth" # 模型保存路径
...
#         if n > 3:  # 只显示前4个样本的预测结果
#             break
#         x = x.to(device)
#         predict = torch.argmax(cnn(x[0].unsqueeze(0)), dim=1)
#         plt.figure(n)
